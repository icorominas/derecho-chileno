...
interface CaseHistoryDetailModalProps {
    caseData: CompletedCase | null;
    onClose: () => void;
}

const CaseHistoryDetailModal: React.FC<CaseHistoryDetailModalProps> = ({ caseData, onClose }) => {
    if (!caseData) {
        return null;
    }

    const { caseDetails, evaluation } = caseData;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-slate-700">
                    <h2 className="text-2xl font-bold text-center text-blue-300 font-serif">Evaluación del Caso</h2>
...
export default CaseHistoryDetailModal;
